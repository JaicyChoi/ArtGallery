...
for( let i = 0 ; i < DATA.ETC.length ; i++ ){
    ...
    img.src = 'img/etc/' + DATA.ETC[i].file + '.png';
    img.alt = DATA.ETC[i].nickname + ' 님 그림';
    hover_info_date.innerHTML = DATA.ETC[i].date;
    hover_info_nickname.innerHTML = DATA.ETC[i].nickname;
...
wrapper_li.forEach( (li, index) => li.addEventListener('click', () => {
    ...
    img.src = 'img/etc/' + DATA.ETC[index].file + '.png';
    img.alt = DATA.ETC[index].nickname + '님 그림';
    show_pic_info_date.innerHTML = DATA.ETC[index].date;
    show_pic_info_nickname.innerHTML = DATA.ETC[index].nickname;
